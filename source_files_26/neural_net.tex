...
\begin{example}[Two-layer ReLU networks with second-layer fixed]
    ...
    Note that $w$ is rotation-invariant. Thus, we can essentially project $x, x'$ to a two-dimensional plane, and instead imagine that we are drawing $w$ from a unit circle.
...
The dynamics of network outputs are governed by the following approximation using the NTK matrix $H^{\star}$:
