...
\begin{example}[Spiked covariance]
    ...
    As a result, the excess risk goes to zero even though the model perfectly interpolated noisy data.\footnote{See also the celebrated Marchenko–Pastur semi-circle law, and its applications in wireless communication \citep{tulino2004random}.}
\end{example}
...
\begin{align}
    \exarg{h \sim \cQ}{L(h)} \le \exarg{h \in \cQ}{\hat L(h)} + \sqrt{\frac {KL(\cQ || \cP) + \log(2n \delta^{-1})} {2(n-1)}}, \label{eq_pac_bayes}
\end{align}
...
Here, instead, one can show a statement on the ``titled'' loss gap as
\begin{align}
    \exarg{S}{\exp(a ( L(f) - \beta \hat L(f)) )} \le \exp\left( \frac {a^2 \beta^2 C^2} {8n} \right) \label{eq_titled_exp}
\end{align}
...
\begin{align*}
    ...
    &\le \exp(a (1 - \beta) L(f)) \cdot \exp\left(\frac{a^2 \beta^2 C^2}{8n}\right) \\
    &\le \exp\left( a(1 - \beta) C + \frac{a^2 \beta^2 C^2}{8n} \right) % \exarg{S}{\exp(a \beta (L(f) - \hat L(f)))}
\end{align*}
By setting $a = O((1 - \beta) n / C)$, we can derive equation \eqref{eq_titled_exp}.
The rest of the proof is based on applying the Donsker-Varadyan change-of-measure, and then turn the expectation into a high-probability bound.
