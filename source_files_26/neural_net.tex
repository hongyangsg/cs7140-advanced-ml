...
\begin{example}[KL divergence between two Gaussian distributions]\label{ex_kl_gauss}
...
\begin{example}[Shifted Gaussians]
    Let $\cP = \cN(0, 1)$, $\cQ = \cN(\mu, 1)$.
    Consider the test function $f(x) = t x$ for some real value $t$.
    The DV inequality \eqref{eq_dv} states that
    \[ \exarg{\cQ}{tx} \le D_{KL}(\cQ || \cP) + \log \exarg{\cP}{e^{tx}} \]
    To compute each term, we note that the left-hand side is $t \mu$.
    For the right hand side, by Example \ref{ex_kl_gauss}, the KL divergence term is $\mu^2 / 2$.
    The second term is $e^{t^2 / 2}$ based on the moment-generating function of a standard Gaussian.
    Putting together, \eqref{eq_dv} is equivalent to $2 t \mu \le \mu^2 + t^2$, which recovers a simple quadratic bound.

    As a remark, the equality holds when $\cQ = \cP_G$.
    In the above example, when $t = \mu$, or when $\cQ = \cN(t, 1)$.
\end{example}
...
This summarizes the high-level proof idea. For a complete proof. please refer to the work of \citet{zhang2024noise}.
\end{proof}