...
\[ u_{t+1} = u_t - \eta XX^\top(u_t - y) \]
Subtracting $y$ from both sides, the residual error $r_t = u_t - y$ evolves as:
\[ r_{t+1} = (I - \eta K)r_t \]
where $K = XX^\top \in \mathbb{R}^{n \times n}$.
Thus, if the step size satisfies $\eta < 2/\lambda_{\max}(K)$, then the residual error at step $t$ satisfies:
\[ \|u_t - y\|^2 \le (1 - \eta \lambda_{\min}(K))^{2t} \|u_0 - y\|^2 \]
In the over-parameterized case ($d > n$), if the rank of $X$ is full (equal to $n$), then $\lambda_{\min}(K) > 0$, and the training error converges to zero at a geometric rate.
\end{example}
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\linewidth]{figures/spectral_gap.png}
...
\begin{example}[Spiked covariance]
    ...
    As a result, the excess risk goes to zero even though the model perfectly interpolated noisy data.
\end{example}
...
\subsection{PAC-Bayes generalization bounds (Lectures 13-14)}
