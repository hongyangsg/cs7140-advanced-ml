...
A reward function $r: \cS \times \cA \rightarrow [0, 1]$, $r(s, a)$, denotes the (immediate) reward associated with taking action $a$ in state $s$.
More generally, $r(s, a)$ could be a random variable where the distribution depends on $s, a$.
A discount factor $\gamma \in [0, 1)$ defines the (expected) horizon for the problem.

Finally, we assume an initial state distribution $\mu \in \Delta(\cS)$, which specifies how the initial state $s_0$ is generated.
...
For a fixed policy and a starting state $s_0 = s$, we define the value function $V^{\pi}: \cS \rightarrow \real$ as the discounted sum of future rewards
\[ V^{\pi}(s) = \ex{\sum_{t=0}^{\infty} \gamma^t r(s_t, a_t) | \pi, s_0 = s} \]
...
Similarly, the action-value (or $Q$-value) function $Q^{\pi}: \cS \times \cA \rightarrow \real$ is defined as
\[ Q^{\pi}(s, a) = \ex{\sum_{t = 0}^{\infty} \gamma^t r(s_t, a_t) | \pi, s_0 = s, a_0 = a} \]

Given a state $s$, the goal of the agent is to find a (probabilistic or deterministic) policy $\pi$ that maximizes the value, i.e., the optimization problem the agent seeks to solve is:
\[ \max_{\pi} V^{\pi}(s), \]
where the max is over all possibly non-stationary and randomized policies.

Next, let us look at the above problem formulation through several examples.
...
In the context of LLMs trained with RL, the MDP is structured with {states} $s \in \mathcal{S}$ with the prompt plus the sequence of tokens generated so far.
...
%\s{Stationary policies.}
An important concept for studying RL problems is that of a stationary policy, which satisfies the following consistency conditions for all $s\in\cS$ and $a\in\cA$, given $V^{\pi}$ and $Q^{\pi}$:
\begin{align*}
    V^{\pi}(s) &= \exarg{a \sim \pi(\cdot \mid s) }{Q^{\pi}(s, a) } \\
    Q^{\pi}(s, a) &= r(s, a) + \gamma \exarg{s'\sim \Pr(\cdot | s, a)}{V^{\pi}(s')}
\end{align*}
Let us denote $P^{\pi}$ as the transition matrix on state-action pairs induced by a stationary matrix $\pi$, specifically:
\[ P^{\pi}_{(s, a), (s', a')} \define \Pr(s' | s, a) \pi(a' | s'), \text{ for any } s' \in \cS, a' \in \cA \]
In particular, for deterministic policies $\pi$, we have
\begin{align*}
    ...
    \end{cases}
\end{align*}
Let $V^{\pi}$ be a vector of length $\cS$ and $Q^{\pi}, r$ a vector of length $\abs{\cS} \times \abs{\cA}$.
Let $P$ be a matrix where $P_{(s, a), s'} = \Pr(s' | s, a)$.
With this notation, we can verify that
\begin{align*}
    Q^{\pi} = r + \gamma P V^{\pi}, \text{ or equivalently } Q^{\pi} = r + \gamma P^{\pi} Q^{\pi},
\end{align*}
leading to the solution that $Q^{\pi} = (\id - \gamma P^{\pi})^{-1} r$. In particular, this step is based on the following expansion of $Q^{\pi}(s, a)$ on an arbitrary state-action pair $(s, a)$:
\begin{align*}
    Q^{\pi}(s, a) &= r(s, a) + \gamma \sum_{(s', a') \in \cS \times \cA} \Pr(s' \mid s, a) \pi(a' \mid s') Q^{\pi}(s', a')  \tag{$ Q^{\pi} = r + \gamma P^{\pi} Q^{\pi}$} \\
    &= r(s, a) + \gamma \sum_{s' \in \cS} \Pr(s' \mid s, a) \underbrace{ \sum_{a' \in \cA} \pi(a' \mid s') Q^{\pi}(s', a') }_{V^{\pi}(s')} \tag{$ Q^{\pi} = r + \gamma P V^{\pi}$ }
\end{align*}
\hl{As a consequence of this equation, given a policy $\pi$, one can obtain the value function by solving a linear equation.}
...
In particular, the Bellman optimality equations \citep{bellman1956dynamic} give a precise characterization of the optimal value function. In the next section, we will dive into this result.

The above assumes an infinite-horizon, discounted MDP. Alternatively, consider a finite-horizon and time-dependent MDP, $M = (\cS, \cA, \set{P}_h, \set{r}_h, H, \mu)$, specified with a state space $\cS$, an action space $\cA$, a time-dependent transition function $P_h: \cS \times \cA \rightarrow \Delta(S)$, a time-dependent reward function $r_h: \cS \times \cA \rightarrow [0, 1]$, an integer $H$ which defines the horizon of the problem, and an initial state distribution $\mu \in \Delta(\cS)$ which specifies how the initial state $s_0$ is generated.
...
\begin{example}
    The difference between finite-horizon vs. infinite-horizon MDPs is that time matters.
    In particular, the optimal policy can be time-dependent, and you may delay actions because horizon is limited, as a result, the value function depends on the episode $h$.

    On the other hand, for infinite-horizon MDPs, the optimal policy is stationary.
\end{example}

\subsection{Value iteration and policy iteration (Lecture 16)}

For an infinite-horizon, discounted MDP, solving the optimal policy can be achieved in polynomial time using constrained optimization \citep{lan2023policy}.
An important concept for computing the optimal value function is called the \emph{Value Iteration}, an iterative method to compute the \emph{optimal} value function $V^{\star}$.
...
\[ \pi^{\star}(s) = \arg\max_{a \in \cA} \left( r(s, a) + \gamma \sum_{s' \in \cS} \Pr[s' \mid s, a] V^\star(s') \right). \]

Another fundamental concept, building on the Bellman's optimality condition, is called the \emph{Policy Iteration}.
We restrict to deterministic policies here, and in particular, policy iteration alternates between two steps until the policy $\pi$ converges to $\pi^\star$:
\begin{itemize}
    \item \emph{Policy evaluation:} Given a policy $\pi_k$, compute the value function $V^{\pi_k}$ by solving the linear system for all $s \in \mathcal{S}$
...
\subsection{Sample complexity and online learning (Lecture 16)}

For many sequential decision-making problems, we may not know the MDP a priori.
Consider an online learning scenario.
...
At every iteration $t$, the learner picks an arm $I_t \in [1, 2, \dots, K]$ according to a certain policy.
An important concept is the cumulative regret with respect to the selected arms:
...
Next, we shall describe the upper confidence bound (UCB) algorithm, which is a near-optimal procedure to identify the best arm \cite{daniel2018tutorial}.
For simplicity, we allocate the first $K$ rounds to pull each arm once.
Then, for $t = 1 \rightarrow T - K$, execute arm 
\begin{align}
    I_t = \arg \max_{i \in [K]} \left( \hat \mu_i^t + \sqrt{\frac {\log(TK / \delta)} {N_i^t}} \right), \text{ and observe } r_t \define r_{I_t}, \label{eq_It}
\end{align}
...
In particular, the second term in equation \eqref{eq_It} represents a confidence penalty inversely proportional to the empirical counts of pulling the $i$-th arm.

The upper confidence bound lemma \citep{foster2023foundations} states that with probability at least $1 - \delta$,
\[ \bigabs{ \hat \mu_a^t - \mu_a } \le 2 \sqrt{ \frac {\ln(TK / \delta)} {N_a^t}  }, \]
where $\hat \mu_a^t$ is the empirical mean for each arm.
It can be shown that the UCB algorithm achieves a cumulative regret of at most $O(\sqrt{K T \ln (KT / \delta)} + K)$, which vanishes relative to the total reward as $T$ increases.
