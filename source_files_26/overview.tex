...
\subsubsection{Uniform convergence and generalization}
...
\paragraph{Language models:} A language model specifies a conditional probability distribution $\Pr_{\theta}(\cdot \mid P)$, given a prompt sequence $P$, produces the next-token according to underlying probability masses.
...
\begin{example}[In-context learning]
    ...
    In this problem, each prompt $P_{\theta_i}$ involves a sequence of examples or demonstrations $(x_1, y_1, x_2, y_2, \dots, x_{t-1}, y_{t-1}, x_t)$, ended with a query example $x_t$.
    The goal is to predict the correct output $y_t$ corresponding to $x_t$.
...
In the next lecture, we will wrap up this overview by providing a setup for rigorously modeling several emerging learning paradigms.
