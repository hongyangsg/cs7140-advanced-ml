...
We train a regression model with word frequencies as features and obtain a training accuracy of 100\% on 1000 training documents and a test accuracy of 85\% on 1000 test documents.
How can we reduce the gap between the training accuracy and the test accuracy?
Further, what happens if the word frequencies between the training corpus and the test corpus are different?
It is possible to answer these questions from a engineering perspective; instead, this course will mostly focus on the mathematical analysis underlying these procedures, although we will provide computational examples from time to time to help you understand the theoretical concepts.

There is obviously a clear gap between theoretical analysis and the practical performance of an algorithm.
For instance, theoretical analysis is usually conducted under standard technical assumptions, which are often made to simplify the theoretical analysis.
The goal, instead, is to \emph{build a deeper understanding of the underlying keep concepts through mathematical modeling and theoretical analysis}.
...
\begin{example}[Linear regression]
    ...
    In a standard parametric regression setup, we have $n$ samples $(x_1, y_1), (x_2, y_2), \dots, (x_n, y_n)$.
\end{example}
