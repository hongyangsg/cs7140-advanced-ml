...
\subsection{Transfer learning linear regression (Lecture 15)}
...
The proof of the above is based on algebraic calculations.
...
\begin{figure*}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/motivation.pdf}
...
\begin{example}[Estimating the mean of a Gaussian]
    Suppose you observe $X_1, \dots, X_n$ from $\cN(\theta, 1)$, where $\theta \in \real$ is unknown, and you want to estimate $\theta$ under squared error loss $(\hat \theta - \theta)^2$.

    The minimax risk is given by \[ \inf_{\hat \theta}\sup_{\theta \in \real} \exarg{\theta}{(\hat \theta - \theta)^2} \]
    A minimax lower bound shows that no estimator can make this risk arbitrarily small. In fact, one can show that the above is at least $\frac 1 n$.
    In particular, the sample mean achieves risk $1/ n$ (due to variance), matching the minimax rate.
\end{example}

In the context of transfer learning linear regression, suppose we are trying to estimate an unknown parameter denoted as $\beta^{(2)}$.
...
This result provides a way to quantify the optimality of a statistical estimation algorithm.
It turns out that the naive estimator described above is nearly optimal, with one caveat.
...
The proof of the minimax lower bound is via a covering argument.
...
\subsection{Multitask and meta-learning (Lecture 15)}

In multitask learning, one is concerned with $k$ data distributions, $\cD_1, \cD_2, \dots, \cD_k$.
The goal is to learn a predictor (or a small number of predictors) that performs well on all the $k$ distributions simultaneously.

Meta-learning is another form of learning where you learn a meta-model during the training stage.
At test time, you want to quickly adapt the meta-model to new samples.

Few-shot learning is a setting where a model must learn or adapt from only a small number of examples.
For example, suppose you want a model to recognize a new class, like a rare bird species, but you only have $5$ labeled images of it. If the model can still do well, this is called few-shot learning.

Many recent papers have sought to theoretically formalize the above learning paradigms. See, e.g.,  \citep{wu2020understanding, finn2019online, du2020few}.
...
\begin{figure}[!t]
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=0.98\textwidth]{figures/multitask_width.pdf}
		\caption{Varying the rank of the shared matrix $B$}
		\label{fig_sec4_width}
	\end{subfigure}
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=0.98\textwidth]{figures/multitask_transfer.pdf}
