...
\begin{itemize}
    \item (5 points) Suppose you are conducting a poll to forecast the upcoming election. Assume you randomly sample 5,000 likely voters, what is the probability that the percentage who support candidate $X$ that you compute differs from the true population value (the expected value) by more than an additive $\pm 2\%$? What about differing by more than $\pm 5\%$ additive? What about deviating by more than $\pm 10\%$?
...
\begin{enumerate}[label=(\alph*)] 
    \item\texttt{[10 points]} Load the MNIST dataset from pytorch. Here is an example code snippet for loading the dataset into pytorch. 
...
\[ H_{i,j}^\star = x_i^Tx_j\cdot \mathbb{E}_{w\sim \cN(0, \id)}[\sigma'(w^Tx_i)\cdot\sigma'(w^Tx_j)]. \]
When $\sigma(z) = \max (0,z)$ is the ReLU function, show that
\[ H_{i,j}^\star = x_i^Tx_j\cdot \frac{\pi - \theta}{2\pi}, \]
