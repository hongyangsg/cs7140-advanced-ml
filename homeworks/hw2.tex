...
\begin{document}
\lecture{Problem Set 2}{\textbf{March 9, 2026, 11:59pm}}
...
\begin{enumerate}[label=(\alph*)]
    ...
    Specifically, specify two fully-connected layers when you set up the network architecture in PyTorch initialization.
...
\begin{itemize}
    \item (10 points) When $\sigma(z) = \max (0,z)$ is the ReLU function, show that
...
%Examine the $H^{\star}$ matrix and comment on your findings.
    ...
    \item (10 points) Next, describe how to apply this kernel to solve a regression problem.

    \item (5 bonus points) When the size of the dataset is too large, the kernel matrix $K$ can become too expensive to compute, as it requires computing every entry of an $n$ by $n$ matrix. Instead, an alternative idea to solve this problem is via dimensionality reduction, otherwise known as random projection. Can you work out the detailed procedure for applying random projection to kernel ridge regression?
\end{itemize}
...
\begin{itemize}
    \item (10 points) For a positive semidefinite matrix $X$, verify that the nuclear norm of $X$ is equal to the trace of $X$.

    \item (20 points) For this part, you can work on a synthetic dataset.
    ...
    Specifically, run the same fitting procedure three times, and report the mean and the standard deviation of the training error (on the observed entries), and the reconstruction error (between the fitted model and the ground-truth matrix).

    [Hint: use the fact that the gradient of the nuclear norm of a matrix $X$ is equal to $U V^{\top}$, where $X = U D V^{\top}$ is the singular value decomposition of a matrix $X$. Specifically, the dimensions of the three matrices are: $U \in \real^{d_1 \times r}$, $D \in \real^{r \times r}$, and $V \in \real^{d_2 \times r}$.]
\end{itemize}
