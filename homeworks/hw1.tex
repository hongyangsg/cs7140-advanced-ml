...
\begin{enumerate}
    ...
    \item (10 points) Next, implement the soft transfer or soft parameter sharing (SPS) estimator with an additional regularization penalty placed on $z$ in the objective function, which determines the difference between models that we will use for task $1$ vs. task $2$.
    In particular, $z$ is a variable that you will optimize during the training process.
